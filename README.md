...
代码参考了[Flutter接入移动解析HTTPDNS Android/iOS SDK实践方案](https://help.aliyun.com/zh/dns/httpdns-practice-solution-of-connecting-flutter-to-alibaba-cloud-public-dns-android-ios-sdk)和已经标记为**discontinued**（截止2025-09-01）的[aliyun_httpdns](https://pub.dev/packages/aliyun_httpdns)。
...
- Flutter 应用中的各种 HTTP 客户端（Dio、HttpClient 等）发起网络请求
- 请求被配置为通过本地代理服务器（localhost:4041）

### 2. **代理服务器处理**

- **请求解析器**：解析 HTTP/HTTPS 请求，提取目标域名和端口
- **协议检测器**：自动识别请求协议类型（HTTP/1.1、HTTP/2、HTTPS）
...
- **连接管理**：管理 Socket 连接，支持连接复用和错误重试
- **错误处理**：处理网络错误、超时、重试等异常情况，返回布尔值而非抛出异常
...
如果你的应用需要支持 HTTP 协议（非HTTPS），必须在 `ios/Runner/Info.plist` 文件中添加以下配置：
...
```dart
// 1. 启动代理服务器（仅在需要代理时启动）
final success = await dnsService.startProxy();
if (success) {
  print('代理服务器启动成功');
} else {
  print('代理服务器启动失败');
...
final success = await dnsService.configureHttpClient(client);
if (success) {
  print('HttpClient配置成功');
} else {
  print('HttpClient配置失败');
}
...
final success = await dnsService.startProxy(proxyConfig);
if (success) {
  print('代理服务器启动成功');
} else {
  print('代理服务器启动失败');
...
final dnsConfig = DnsConfig(
  ...
  maxNegativeCache: 30,        // 最大否定缓存时间（秒）
  enableSpeedTest: true,       // 启用测速
  timeout: 5,                  // 超时时间（秒）
  enableIPv6: false,           // 是否启用IPv6
  enableShort: false,          // 是否启用短连接
  preloadDomains: [           // 预加载域名
    'www.example.com',
    'api.example.com',
  ],
  keepAliveDomains: [         // 缓存保持域名
    'www.example.com',
  ],
  maxCacheTTL: 3600,          // 最大缓存TTL时间（秒）
  ispEnable: true,            // 启用ISP网络区分
  speedPort: 80,              // 测速端口
);
...
// 停止代理
final success = await dnsService.stopProxy();
if (success) {
  print('代理服务器已停止');
} else {
  print('停止代理服务器失败');
...
class NetworkService {
  ...
  Future<void> makeHttpRequest() async {
    ...
    final client = HttpClient();
    final success = await _dnsService.configureHttpClient(client);
    if (success) {
      // 发起请求...
    } else {
      print('配置HttpClient失败');
    }
  }
...
当需要绕过HTTPDNS直接使用系统DNS解析时，可以使用 `resolveWithSystemDns` 方法：

```dart
// 使用系统DNS解析域名（默认只返回公网IP）
final ip = await dnsService.resolveWithSystemDns('www.example.com');
if (ip != null) {
  print('系统DNS解析结果: $ip');
} else {
  print('系统DNS解析失败');
...
// 对比HTTPDNS和系统DNS的解析结果
final httpDnsResult = await dnsService.resolveDomain('www.example.com');
final systemDnsResult = await dnsService.resolveWithSystemDns('www.example.com');

print('HTTPDNS结果: $httpDnsResult');
print('系统DNS结果: $systemDnsResult');

// 调试模式：获取所有IP地址（包括私有地址）
final allIps = await dnsService.resolveWithSystemDns('www.example.com', onlyPublicIp: false);
print('所有IP地址: $allIps');
```

**参数说明：**
- `domain`: 要解析的域名
- `onlyPublicIp`: 是否只返回公网IP地址，默认为 `true`
  - `true`: 过滤私有地址、回环地址、多播地址等，只返回可公网访问的IP
  - `false`: 返回所有解析到的IP地址，包括私有地址（用于调试）
...
| `maxNegativeCache`| `int`          | `30`    | 最大否定缓存时间（秒） |
| `enableSpeedTest` | `bool`         | `true`  | 是否启用速度测试 |
| `timeout`         | `int`          | `3`     | 超时时间（秒）   |
| `enableIPv6`      | `bool`         | `false` | 是否启用 IPv6    |
| `enableShort`     | `bool`         | `false` | 是否启用短连接   |
| `preloadDomains`  | `List<String>` | `[]`    | 预加载域名列表   |
| `keepAliveDomains`| `List<String>` | `[]`    | 缓存保持域名列表 |
| `maxCacheTTL`     | `int`          | `3600`  | 最大缓存 TTL 时间（秒） |
| `ispEnable`       | `bool`         | `true`  | 是否启用 ISP 网络区分 |
| `speedPort`       | `int`          | `80`    | 测速端口         |
...
// 使用系统DNS解析域名（绕过HTTPDNS），默认过滤非公网地址
Future<String?> resolveWithSystemDns(String domain, {bool onlyPublicIp = true})
```
...
A: 插件提供基础的连接管理功能，包括连接复用和错误重试，确保网络请求的稳定性和性能。连接管理采用简化的实现方式，专注于核心的网络代理功能。
...
A: 插件利用阿里云 HTTPDNS 的基础负载均衡能力，通过智能的 IP 选择来提升网络请求的性能和稳定性。负载均衡功能主要依赖阿里云 HTTPDNS 服务的原生能力。

### Q: 支持哪些 HTTP 客户端？

A: 支持 Dio、HttpClient 等主流 HTTP 客户端，支持 HTTP/1.1 和 HTTP/2 协议，也可以自定义配置其他客户端。目前主要专注于 HTTP/HTTPS 协议的支持。

### Q: 什么时候应该初始化插件？

A: 建议在用户同意隐私协议之后，第一次请求网络之前进行初始化。这样可以确保在用户同意的情况下才开始网络活动。初始化后可以通过检查返回值来判断是否成功。

### Q: 什么时候应该启动代理服务器？

A: 代理服务器应该按需启动。如果只需要域名解析功能，无需启动代理；只有在需要代理 HTTP 请求时才启动代理服务器。启动后可以通过检查返回值来判断是否成功。
...
A: 不需要。域名解析功能不需要代理服务器，只有 HTTP 请求代理才需要启动代理服务器。域名解析方法会返回解析结果或原域名，不会抛出异常。

### Q: 如何使用系统DNS解析？

A: 使用 `resolveWithSystemDns(String domain, {bool onlyPublicIp = true})` 方法可以直接使用系统DNS解析域名，绕过HTTPDNS服务。默认情况下只返回公网IP地址，过滤掉私有地址、回环地址等无效地址。设置 `onlyPublicIp: false` 可以获取所有IP地址用于调试。这个方法适用于网络调试、对比测试或当HTTPDNS服务不可用时的备选方案。解析成功返回IP地址，失败返回null。
